...
public class TemporaryStorageDAOImpl implements TemporaryStorageDAO {
	...
	public ResponseEntity<Object> getTemporaryStorage(String fromDate, String toDate, String currentUIDate,
		...
		final List<TemporaryStorage> lstUTCConvertedDate = objMapper
				.convertValue(
						dateUtilityFunction.getSiteLocalTimeFromUTC(lstTemporaryStorage,
								Arrays.asList("sstoragedatetime"),
								null, userInfo, false, null, false),
						new TypeReference<List<TemporaryStorage>>() {
						});
		if (!lstTemporaryStorage.isEmpty()) {
			outputMap.put("TemporaryStorage", lstUTCConvertedDate);
...
//			}
			...
			objMapper.registerModule(new JavaTimeModule());			
		     ...
		     final TemporaryStorage objTS = new TemporaryStorage();			
			 String dCollectionDateTime=(String) temporaryStorageObj.get("dstoragedatetime");
		     Instant  instantCollectionDateTime =Instant.parse(dCollectionDateTime);	     
 
		     objTS.setDstoragedatetime(instantCollectionDateTime);		
			
			 short ntzcollectiondatetime = ((Integer) temporaryStorageObj.get("ntzstoragedatetime")).shortValue();
			
			 objTS.setNtzstoragedatetime(ntzcollectiondatetime);
			
			 objTS.setSstoragedatetime(dateUtilityFunction.instantDateToString(objTS.getDstoragedatetime()).replace("T", " ").replace("Z", ""));		
			
			final TemporaryStorage convertedObject = objMapper.convertValue(
					dateUtilityFunction.convertInputDateToUTCByZone(objTS,
							Arrays.asList("sstoragedatetime"), Arrays.asList("ntzstoragedatetime"), true, userInfo),
			...
			final String insertQuery = "insert into temporarystorage(ntemporarystoragecode,nprojecttypecode,sbarcodeid,jsondata,dstoragedatetime,ntzstoragedatetime,noffsetdstoragedatetime,"
					...
					+ temporaryStorageObj.get("jsondata") + "','" + convertedObject.getSstoragedatetime()+ "',"
					+ temporaryStorageObj.get("ntzstoragedatetime") + "," + "'"
					+ convertedObject.getNoffsetdstoragedatetime() + "',N'"
	...
	public ResponseEntity<Object> updateTemporaryStorage(final Map<String, Object> inputMap, final UserInfo userInfo)
		...
		if (activeSampleCollection != null) {
			...
			final String updateQuery = "update temporarystorage set dstoragedatetime='"
					...
					+ objTemporaryStorage.getNoffsetdstoragedatetime() + ",scomments=N'"
